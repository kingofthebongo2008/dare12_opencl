...
namespace freeform
{
    namespace details
    {
        #include "opencl_freeform_bitonic_sort_by_key_compiled.h"

        inline std::tuple<const uint8_t*, size_t> get_freeform_bitonic_sort_by_key_ir(void)
        {
            return std::make_tuple(reinterpret_cast<const uint8_t*> (&opencl_freeform_bitonic_sort_by_key[0]), sizeof(opencl_freeform_bitonic_sort_by_key) );
        }
    ...
    inline std::unique_ptr< opencl::program>  create_freeform_bitonic_sort_by_key_kernel(const opencl::context * ctx )
    {
        auto program = details::create_freeform_bitonic_sort_by_key_kernel(*ctx, ctx->get_device());
