...
namespace freeform
{
    ...
    inline shader_bezier_ps   create_shader_bezier_ps(ID3D11Device* device)
    {
        return shader_bezier_ps(details::create_shader_bezier_ps(device));
    }

    inline std::future< shader_bezier_ps> create_shader_bezier_ps_async(ID3D11Device* device)
    {
        return std::async(std::launch::async, create_shader_bezier_ps, device);
    }
}
